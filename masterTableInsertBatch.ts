...
// Function to batch insert rows
async function batchInsert(rows, batchSize = 1000) {
  ...
  for (let i = 0; i < totalRows; i += batchSize) {
    ...
    await db.insert(importedMasterTable).values(values);
  }
}

// Batch insert the rows from the dataframe
(async () => {
  const rows = USAPL.rows();
